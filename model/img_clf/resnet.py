...
class _ResNeXt_50(Estimator, NeuralModel, ImageClassifier):
    ...
    def build_model(self) -> None:
        ...
        self.layer_2, in_channels = F.make_res_layers(
            64, 64, Bottleneck, 3, 2, base_args, res_args, 1, layer_label
        )
        self.layer_3, in_channels = F.make_res_layers(
            in_channels, 128, Bottleneck, 4, 3, base_args, res_args, 2, layer_label
        )
        self.layer_4, in_channels = F.make_res_layers(
            in_channels, 256, Bottleneck, 6, 4, base_args, res_args, 2, layer_label
        )
        self.layer_5, in_channels = F.make_res_layers(
            in_channels, 512, Bottleneck, 3, 5, base_args, res_args, 2, layer_label
        )
